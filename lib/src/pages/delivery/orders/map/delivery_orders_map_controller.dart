...
class DeliveryOrdersMapController {
  ...
  void saveLocation()async{
    ...
    await _ordersProvider.updateLatLngDB(order);
  ...
  void emitPosition(Position _positiond) {

    /*var coords={'id_order': order.id,
    'lat': _position?.latitude,'lng': _position?.longitude};
    socket.emit('position',jsonEncode(coords));*/
   socket.emit('position', {
      'id_order': order.id,
      'lat': _positiond.latitude,
      'lng': _positiond.longitude,
    });
  }
  ...
  void updateToDelivered() async{
      ...
      if(_distanceBetween<=200){
        ResponseApi responseApi=await _ordersProvider.updateToDeliveredDB(order);
    ...
    Fluttertoast.showToast(msg: 'Debes estar mas cerca a la posición de entrega');
